...
/* Page directory and Page table values */
...
// struct for a page directory pointing to a 4kb page table that points to 4 kb pages
typedef struct page_directory_4kb {
    union {
        ...
        struct {
            ...
            uint32_t avail : 3;
            uint32_t bit_20_addr : 20;
        } __attribute__((packed));
    };
} page_directory_4kb;

//page table pointing to pages of 4 kb
typedef struct page_table_4kb {
    union {
    uint32_t val;
        struct {
            uint32_t present : 1;
...
//page directory pointing to pages that are 4 mb large 
typedef struct page_directory_4mb {
    union {
    uint32_t val;
        struct {
            uint32_t present : 1;
            ...
            uint32_t attribute_index : 1;
            uint32_t reserved : 9;
            uint32_t bit_10_addr : 10;
        } __attribute__((packed));
    };
} page_directory_4mb;

typedef union page_directory_entry {
    page_directory_4kb directory_entry_4kb;
    page_directory_4mb directory_entry_4mb;
} page_directory_entry;

page_directory_entry page_directory[PD_NUM_ENTRIES]__attribute__((aligned (SIZE_4KB)));
page_table_4kb page_table[PT_NUM_ENTRIES]__attribute__((aligned (SIZE_4KB)));
//terminal switches paging  
page_table_4kb page_table_terminal[PT_NUM_ENTRIES]__attribute__((aligned (SIZE_4KB)));
